...
namespace WPF_N_Tier_Test.ViewModel.Sales.Customers
{
    public partial class OrderDetailsViewModel : BaseModel
    {
        ...
        public bool IsOrderEligibleToValidate => !(SelectedOrder?.IsValidated) ?? false;
        public bool IsOrderEligibleToPay => !SelectedOrder?.IsPaid ?? false && (SelectedOrder?.IsValidated ?? false);
        public bool IsOrderEligibleToRevoke => SelectedOrder?.IsValidated ?? false;

        public string? InitialDate => SelectedOrder?.DateTime.ToString("dd/MM/yy H:mm") ?? "Unknown";
        public string? ValidationDate => SelectedOrder?.ValidationDate?.ToString("dd/MM/yy H:mm") ?? "_ _";
        ...
        {
            if (!IsOrderEligibleToValidate)
                return;
            Parent.IsBusy = true;
            var result = await Parent.customersService.VALIDATE_Order(OID);
            if (result != null)
            {
                Parent.SelectedOrder.IsValidated = true;
